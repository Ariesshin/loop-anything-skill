...
Most READMEs make you click away before showing proof. Here are the results first.

We ran controlled README-style experiments where the same initial draft was saved as the baseline, then sent through Loop review until every final reviewer returned `PASS 120`.

The three cases cover writing, visual design, and small-tool reliability.

| Case | Before | After Loop | Delta | Loop status | Evidence |
|---|---:|---:|---:|---|---|
| 2026 National I Gaokao essay | 78.2 | 100.0 | +21.8 | `full_pass` | [comparison](experiments/readme-before-after-suite/experiments/01-gaokao-essay/evaluation/comparison.md) |
| Standalone HTML aesthetic optimization | 67.3 | 100.0 | +32.7 | `full_pass` | [comparison](experiments/readme-before-after-suite/experiments/02-landing-page/evaluation/comparison.md) |
| Receipt splitter reliability | 45.5 | 100.0 | +54.5 | `full_pass` | [comparison](experiments/readme-before-after-suite/experiments/03-receipt-splitter/evaluation/comparison.md) |

**HTML page before vs after Loop:**

| Before: usable, but visually generic | After Loop: same content, stronger visual craft |
|---|---|
| ![Before Loop HTML landing page screenshot](experiments/readme-before-after-suite/evidence/screenshots/02-html-before-readme.png) | ![After Loop HTML landing page screenshot](experiments/readme-before-after-suite/evidence/screenshots/02-html-after-readme.png) |

**What changed:**

- The Gaokao essay moved from a generic era paragraph to a concrete student growth argument.
- The HTML page kept the same product content, but changed from flat and conservative to visually distinctive.
- The receipt splitter moved from passing simple cases to obeying named money rules: discounts before tax/tip, exact cents, subset sharing, and invalid input rejection.

Full bilingual evidence:

- [Experiment report](experiments/readme-before-after-suite/README.md)
- [Before / After gallery](experiments/readme-before-after-suite/summary/before-after-gallery.md)
- [Scores and metrics](experiments/readme-before-after-suite/summary/metrics.json)

Note: these are demonstration experiments, not statistical proof. Scores are demonstration scores, and token counts are visible-text estimates, not billing data.
...
$skill-installer install https://github.com/Ariesshin/loop-anything-skill
```
→ Same `$skill-installer` command works in OpenClaw-compatible agents. Also available on [ClawHub](https://clawhub.ai/Ariesshin/loop-anything-skill).
...
好的 README 不应该让读者先点进报告才看到结果。这里先把结果直接放出来。

我们做了三组 README 展示型控制实验：同一份 AI 初稿保存为 baseline，另一份进入 Loop 审查，直到最终每个审查员都返回 `PASS 120`。

三组案例分别覆盖写作、视觉设计和小工具可靠性。

| 案例 | Loop 前 | Loop 后 | 提升 | Loop 状态 | 证据 |
|---|---:|---:|---:|---|---|
| 2026 全国 I 卷高考作文 | 78.2 | 100.0 | +21.8 | `full_pass` | [对比](experiments/readme-before-after-suite/experiments/01-gaokao-essay/evaluation/comparison.md) |
| 单文件 HTML 美学优化 | 67.3 | 100.0 | +32.7 | `full_pass` | [对比](experiments/readme-before-after-suite/experiments/02-landing-page/evaluation/comparison.md) |
| 账单拆分工具可靠性 | 45.5 | 100.0 | +54.5 | `full_pass` | [对比](experiments/readme-before-after-suite/experiments/03-receipt-splitter/evaluation/comparison.md) |

**HTML 页面 Loop 前后对比：**

| Loop 前：可用，但视觉保守 | Loop 后：同内容，更有视觉完成度 |
|---|---|
| ![Loop 前 HTML 落地页截图](experiments/readme-before-after-suite/evidence/screenshots/02-html-before-readme.png) | ![Loop 后 HTML 落地页截图](experiments/readme-before-after-suite/evidence/screenshots/02-html-after-readme.png) |

**变化重点：**

- 高考作文从泛化时代段落，变成有具体学生成长细节和论证桥的文章。
- HTML 页面保持同一套产品内容，但从平、保守，变成更有辨识度和产品质感。
- 账单拆分工具从只能通过简单情况，变成能遵守明确的现实金钱规则：折扣先于税费、小数分位精确对齐、部分人共享、非法输入拒绝。

完整双语证据：

- [实验报告](experiments/readme-before-after-suite/README.md)
- [前后对比索引](experiments/readme-before-after-suite/summary/before-after-gallery.md)
- [评分与指标](experiments/readme-before-after-suite/summary/metrics.json)

说明：这些是演示实验，不是统计证明。评分是演示评分，token 数字是可见文本估算，不是账单数据。
...
$skill-installer install https://github.com/Ariesshin/loop-anything-skill
```
→ `$skill-installer` 是 Codex、OpenCode 和 OpenClaw 兼容 Agent 的内置命令，无需单独安装。粘贴到会话中，重启后即可使用。也可在 [ClawHub](https://clawhub.ai/Ariesshin/loop-anything-skill) 搜索安装。
